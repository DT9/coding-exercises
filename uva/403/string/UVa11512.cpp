 ...
 *
 *   Solve for the longest repeating substring
 *
 * Solution Summary:
 *
...
int main() {
    ...
    while(cin >> T){
        ...
        string lrsans = T.substr(SA[ans.second],ans.first);
        if (lrsans.empty()) {
            cout << "No repetitions found!" << endl;
        } else {
            int count = 0;
            size_t nPos = T.find(lrsans, 0); // fist occurrence
            while(nPos != string::npos)
            {
                count++;
                nPos = T.find(lrsans, nPos+1);
            }
            cout << lrsans << " " << count << endl;
        }
    };
    return 0;
}
